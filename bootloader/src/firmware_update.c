...
void firmware_update_debug_checksum_calculation(void) {
    ...
    SYS_CONSOLE_PRINT("firmware_update: fnv_32a_hash result: 0x%08x\r\n", checksum1);
    ...
    SYS_CONSOLE_PRINT("firmware_update: manual FNV-32a result: 0x%08x\r\n", checksum2);
...
bool firmware_update_copy_internal_flash_to_external_flash(void){
    ...
    while (remaining > 0) {
        ...
        if (!NVM_Read((uint32_t*)binary_buffer, chunk_size, current_addr)) {
            SYS_CONSOLE_PRINT("firmware_update: failed to read binary chunk at 0x%08x\r\n", current_addr);
            free(binary_buffer);
            return false;
        }
    ...
    SYS_CONSOLE_PRINT("firmware_update: calculated checksum: 0x%08x\r\n", checksum);
    ...
    SYS_CONSOLE_PRINT("firmware_update: erasing %u sectors for binary data\r\n", total_sectors);
    
    for (uint32_t sector = 0; sector < total_sectors; sector++) {
        uint32_t sector_addr = FLASH_BINARY_OFFSET + (sector * sector_size);
        SYS_CONSOLE_PRINT("firmware_update: erasing sector %u at 0x%08x\r\n", sector, sector_addr);
        
        if (!flash_erase_sector(sector_addr)) {
            SYS_CONSOLE_PRINT("firmware_update: failed to erase sector %u\r\n", sector);
            free(binary_buffer);
            return false;
        }
    }
    ...
    while (remaining > 0) {
        ...
        while (!chunk_verified && chunk_retries < max_retries) {
            ...
            if (!flash_write(external_addr, binary_buffer, chunk_size)) {
                SYS_CONSOLE_PRINT("firmware_update: failed to write binary chunk to external flash (retry %u)\r\n", chunk_retries + 1);
                chunk_retries++;
                continue;
            }
            ...
            if (!flash_read(external_addr, verify_buffer, chunk_size)) {
                SYS_CONSOLE_PRINT("firmware_update: failed to read back chunk for verification (retry %u)\r\n", chunk_retries + 1);
                free(verify_buffer);
                chunk_retries++;
                continue;
            }
            ...
            for (uint32_t i = 0; i < chunk_size; i++) {
                if (binary_buffer[i] != verify_buffer[i]) {
                    SYS_CONSOLE_PRINT("firmware_update: verification failed at offset %u: expected=0x%02x, got=0x%02x (retry %u)\r\n", 
        ...
        if (!chunk_verified) {
            SYS_CONSOLE_PRINT("firmware_update: failed to write and verify chunk after %u retries\r\n", max_retries);
            free(binary_buffer);
            return false;
        }
...
bool firmware_update_copy_external_flash_to_internal_flash(void){
    ...
    SYS_CONSOLE_PRINT("firmware_update: external file size: %u bytes\r\n", external_fw_info.file_size);
    SYS_CONSOLE_PRINT("firmware_update: external checksum: 0x%08x\r\n", external_fw_info.checksum);
    ...
    if (external_fw_info.file_size != expected_binary_size) {
        SYS_CONSOLE_PRINT("firmware_update: file size mismatch - expected: %u, got: %u\r\n", 
                         expected_binary_size, external_fw_info.file_size);
        return false;
    }
    
    // Allocate buffer for binary data (use row size for efficiency)
    uint8_t *binary_buffer = malloc(NVM_FLASH_ROWSIZE); // 512 bytes per row
    if (binary_buffer == NULL) {
        SYS_CONSOLE_PRINT("firmware_update: failed to allocate row buffer\r\n");
        return false;
    }
    
    // Calculate checksum of external binary data using FNV-32a
    uint32_t external_checksum = FNV_32_INIT;
    uint32_t external_addr = FLASH_BINARY_OFFSET;
    uint32_t remaining = expected_binary_size;
    
    SYS_CONSOLE_PRINT("firmware_update: calculating checksum of external binary data\r\n");
    
    while (remaining > 0) {
        uint32_t chunk_size = (remaining > NVM_FLASH_ROWSIZE) ? NVM_FLASH_ROWSIZE : remaining;
        
        // Read chunk from external flash
        if (!flash_read(external_addr, binary_buffer, chunk_size)) {
            SYS_CONSOLE_PRINT("firmware_update: failed to read external binary chunk at 0x%08x\r\n", external_addr);
            free(binary_buffer);
            return false;
        }
        ...
        for (uint32_t i = 0; i < chunk_size; i++) {
            external_checksum ^= (uint32_t)binary_buffer[i];
            external_checksum += (external_checksum << 1) + (external_checksum << 4) + 
                               (external_checksum << 7) + (external_checksum << 8) + 
                               (external_checksum << 24);
        }
    ...
    SYS_CONSOLE_PRINT("firmware_update: stored external checksum: 0x%08x\r\n", external_fw_info.checksum);
    
    // Compare checksums
    if (external_checksum != external_fw_info.checksum) {
        SYS_CONSOLE_PRINT("firmware_update: external checksum mismatch - calculated: 0x%08x, stored: 0x%08x\r\n", 
                         external_checksum, external_fw_info.checksum);
        free(binary_buffer);
        return false;
    }
    
    SYS_CONSOLE_PRINT("firmware_update: external checksum verification passed\r\n");
    
    // Now copy the binary data to internal flash
    SYS_CONSOLE_PRINT("firmware_update: copying binary data to internal flash\r\n");
    
    external_addr = FLASH_BINARY_OFFSET;
    uint32_t internal_addr = APPLICATION_START_ADDRESS;
    remaining = expected_binary_size;
    uint32_t total_copied = 0;
    const uint32_t max_retries = 3;
    
    // First, erase all pages that will be written to
    uint32_t pages_to_erase = (expected_binary_size + NVM_FLASH_PAGESIZE - 1) / NVM_FLASH_PAGESIZE;
    uint32_t erase_addr = APPLICATION_START_ADDRESS;
    
    SYS_CONSOLE_PRINT("firmware_update: erasing %u pages starting at 0x%08x\r\n", pages_to_erase, erase_addr);
    
    for (uint32_t page = 0; page < pages_to_erase; page++) {
        // Erase the page
        if (!NVM_PageErase(erase_addr)) {
            SYS_CONSOLE_PRINT("firmware_update: failed to erase page at 0x%08x\r\n", erase_addr);
            free(binary_buffer);
            return false;
        }
        ...
        // Check for errors
        NVM_ERROR error = NVM_ErrorGet();
        if (error != NVM_ERROR_NONE) {
            SYS_CONSOLE_PRINT("firmware_update: page erase error at 0x%08x: 0x%x\r\n", erase_addr, error);
            free(binary_buffer);
            return false;
        }
    ...
    while (remaining > 0) {
        ...
        while (!chunk_verified && chunk_retries < max_retries) {
            ...
            // Pad the buffer to full row size if needed
            if (chunk_size < NVM_FLASH_ROWSIZE) {
                memset(binary_buffer + chunk_size, 0xFF, NVM_FLASH_ROWSIZE - chunk_size);
            }
            
            // Write row to internal flash using NVM_RowWrite
            if (!NVM_RowWrite((uint32_t*)binary_buffer, internal_addr)) {
                SYS_CONSOLE_PRINT("firmware_update: failed to write row to internal flash at 0x%08x (retry %u)\r\n", 
                                 internal_addr, chunk_retries + 1);
                chunk_retries++;
                continue;
            }
            ...
            NVM_ERROR error = NVM_ErrorGet();
            if (error != NVM_ERROR_NONE) {
                SYS_CONSOLE_PRINT("firmware_update: row write error at 0x%08x: 0x%x (retry %u)\r\n", 
                                 internal_addr, error, chunk_retries + 1);
                chunk_retries++;
                continue;
            }
            
            // Verify the written row
            uint8_t *verify_buffer = malloc(NVM_FLASH_ROWSIZE);
            if (verify_buffer == NULL) {
                SYS_CONSOLE_PRINT("firmware_update: failed to allocate verify buffer\r\n");
                free(binary_buffer);
                return false;
            ...
            if (!NVM_Read((uint32_t*)verify_buffer, NVM_FLASH_ROWSIZE, internal_addr)) {
                SYS_CONSOLE_PRINT("firmware_update: failed to read back row for verification at 0x%08x (retry %u)\r\n", 
                                 internal_addr, chunk_retries + 1);
                free(verify_buffer);
                chunk_retries++;
                continue;
            ...
            // Compare written data with original (only the actual data, not padding)
            bool verify_match = true;
            for (uint32_t i = 0; i < chunk_size; i++) {
                if (binary_buffer[i] != verify_buffer[i]) {
                    SYS_CONSOLE_PRINT("firmware_update: verification failed at offset %u: expected=0x%02x, got=0x%02x (retry %u)\r\n", 
            ...
            if (verify_match) {
                chunk_verified = true;
                SYS_CONSOLE_PRINT("firmware_update: row verified successfully\r\n");
            } else {
                chunk_retries++;
                SYS_CONSOLE_PRINT("firmware_update: row verification failed, retrying...\r\n");
            }
        }
        
        if (!chunk_verified) {
            SYS_CONSOLE_PRINT("firmware_update: failed to write and verify row after %u retries\r\n", max_retries);
            free(binary_buffer);
            return false;
        }
        
        external_addr += chunk_size;
        internal_addr += NVM_FLASH_ROWSIZE; // Always advance by full row size
        remaining -= chunk_size;
        total_copied += chunk_size;
    ...
    // Verify the entire copied binary by calculating its checksum
    SYS_CONSOLE_PRINT("firmware_update: verifying entire copied binary\r\n");
    
    uint32_t verify_checksum = FNV_32_INIT;
    uint32_t verify_addr = APPLICATION_START_ADDRESS;
    remaining = expected_binary_size;
    
    // Allocate a smaller buffer for verification
    uint8_t *verify_buffer = malloc(512);
    if (verify_buffer == NULL) {
        SYS_CONSOLE_PRINT("firmware_update: failed to allocate verification buffer\r\n");
        return false;
    }
    ...
    while (remaining > 0) {
        ...
        // Read chunk from internal flash
        if (!NVM_Read((uint32_t*)verify_buffer, chunk_size, verify_addr)) {
            SYS_CONSOLE_PRINT("firmware_update: failed to read internal binary chunk for verification\r\n");
            free(verify_buffer);
            return false;
        }
        ...
        // Update checksum using FNV-32a algorithm
        for (uint32_t i = 0; i < chunk_size; i++) {
            verify_checksum ^= (uint32_t)verify_buffer[i];
            verify_checksum += (verify_checksum << 1) + (verify_checksum << 4) + 
                             (verify_checksum << 7) + (verify_checksum << 8) + 
                             (verify_checksum << 24);
        }
    ...
    SYS_CONSOLE_PRINT("firmware_update: verification checksum: 0x%08x\r\n", verify_checksum);
    SYS_CONSOLE_PRINT("firmware_update: original checksum: 0x%08x\r\n", external_fw_info.checksum);
    
    // Compare verification checksum with original
    if (verify_checksum != external_fw_info.checksum) {
        SYS_CONSOLE_PRINT("firmware_update: final verification failed - checksum mismatch\r\n");
        return false;
    }
    
    SYS_CONSOLE_PRINT("firmware_update: copy completed successfully with full verification\r\n");
    SYS_CONSOLE_PRINT("firmware_update: copied %u bytes from external to internal flash\r\n", total_copied);
...
bool firmware_update_compare_internal_flash_to_external_flash(void){
    ...
    while (remaining > 0 && !data_mismatch) {
        ...
        if (!NVM_Read((uint32_t*)internal_buffer, chunk_size, current_addr)) {
            SYS_CONSOLE_PRINT("firmware_update: failed to read internal binary chunk at 0x%08x\r\n", current_addr);
        ...
        if (chunk_mismatch) {
            SYS_CONSOLE_PRINT("firmware_update: binary data mismatch at offset %u (chunk offset %u)\r\n", 
                             bytes_compared + mismatch_offset, mismatch_offset);
            SYS_CONSOLE_PRINT("firmware_update: mismatch at byte %u: internal=0x%02x, external=0x%02x\r\n", 
    ...
    if (internal_checksum != external_fw_info.checksum) {
        ...
        SYS_CONSOLE_PRINT("firmware_update: checksum mismatch - internal: 0x%08x, external: 0x%08x\r\n", 
