...
#define NETWORK_SEND_STATUS_REQUEST_URI "/STATUS-REQUEST"
#define NETWORK_SEND_TAG_REQUEST_URI "/TAG-REQUEST"

char network_request_json_str[RESOURCE_JSON_STR_SIZE];
char * network_get_status_request_json_str(void){    
    cJSON * root = cJSON_CreateObject();
    cJSON_AddStringToObject(root,"tag",network.tag);
...
char * network_get_tag_request_json_str(void){
    cJSON * root = cJSON_CreateObject();
...
bool network_send_status_request(void){
    ...
    network_send_request_coap_message(
        ...
        network_get_status_request_json_str()
    );
...
bool network_send_tag_request(void){
    SYS_CONSOLE_PRINT("network: send tag request\r\n");
    // 1. Send CoAP message if not MQTT enabled
    network_send_request_coap_message(
        NETWORK_SEND_TAG_REQUEST_URI, 
        network_get_tag_request_json_str()
    );
    // 2. Send MQTT message if MQTT enabled
    // TODO: send mqtt message
    return true;
